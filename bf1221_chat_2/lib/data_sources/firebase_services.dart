...
import 'package:bf1221_chat/data_sources/firebase_repository.dart';
import 'package:bf1221_chat/models/app_user.dart';
import 'package:bf1221_chat/models/message.dart';
import 'package:bf1221_chat/models/room_chat.dart';
...
class FirebaseServices {
  ...
  Future<String?> uploadFile(String userId, File file) async {
    ...
    try {
      final uploaTask =
      await firebaseRepository.firebaseStorage.ref('users/$userId/$fileName.jpg').putFile(file);
  ...
  Future createUserToDatabase(AppUser appUser) async {
    await firebaseRepository.firestore.collection("users").doc(appUser.uid).set(appUser.toJson());
  }

  Future signInWithEmailAndPassword(
      {required String email, required String password}) async {
    try {
      await firebaseRepository.firebaseAuth.signInWithEmailAndPassword(email: email, password: password);
  ...
  Future signInWithGoogle() async {
    UserCredential? user;
    try {
      GoogleSignInAccount? _signInAccount = await firebaseRepository.googleSignIn.signIn();
      if (_signInAccount != null) {
        ...
        user = await firebaseRepository.firebaseAuth.signInWithCredential(credential);
  ...
  Future<AppUser?> getUser(String uid) async {
    final snapshot = await firebaseRepository.firestore.collection("users").doc(uid).get();
    if (snapshot.exists) {
